...
@after {
	...
	(
		...
		{
			$current = $this_SimpleWait_0.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
		...
		{
			$current = $this_ComplexWait_1.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
		...
		{
			$current = $this_SendSignal_3.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
		...
		{
			$current = $this_Cooperate_4.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
		...
		{
			$current = $this_SendMap_7.current;
			afterParserOrEnumRuleCall();
		}
	)
;
...
entrySuperXExpression returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getXbaseXExpressionRule()); }
	iv_superXExpression=superXExpression
	{ $current=$iv_superXExpression.current; }
	EOF;

// Rule XExpression
superXExpression returns [EObject current=null]
@init {
	enterRule();
}
@after {
...
@after {
	...
	(
		...
		(
			('wait' | 'move' | 'send' | 'cooperate' | 'charge' | 'scan' | 'extends' | 'static' | 'import' | 'extension' | '!' | '-' | '+' | 'new' | '{' | 'switch' | 'synchronized' | '<' | 'super' | '#' | '[' | 'false' | 'true' | 'null' | 'typeof' | 'if' | 'for' | 'while' | 'do' | 'throw' | 'return' | 'try' | '(' | RULE_ID | RULE_HEX | RULE_INT | RULE_DECIMAL | RULE_STRING)=>
			(
				{
